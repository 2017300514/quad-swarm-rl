# 中文注释副本；原始文件：paper/mean_std_plots_quad_scale.py
# 该脚本把不同 swarm 规模实验的训练日志聚合成论文对比图。
# 上游输入是若干规模配置目录下的 TensorBoard run；下游输出是 `quads_train_scaling.pdf`，
# 用来对比不同规模设置下平均目标距离和机间碰撞率随训练步数的变化。
...
plt.rcParams['figure.figsize'] = (HALF_PAGE_WIDTH, 2)
...
def extract(experiments):
    ...
    # 这里默认第一条 run 的标量 key 集合作为绘图基准，不再强制所有 run 完全一致。
    ...
    # 原始日志 step 可能不完全对齐，先保留每条 run 的真实 step 轨迹。
    x_per_key = [
    ...
    for plot_idx in range(len(PLOTS)):
        ...
        for run_idx in range(len(values)):
            ...
            for x_idx in x:
                ...
                # 这里做的是粗粒度对齐，而不是精确插值，目标只是让多条 run 能在同一 x 轴上聚合。
                if x_idx == 0:
...
def aggregate(path, subpath, experiments, ax, legend_name, group_id):
    # 每个规模组的中间结果缓存到 `paper/cache/<subpath>/`，方便反复调图时直接复用。
...
def plot(index, interpolated_key, ax, legend_name, group_id):
    ...
    # 目标距离和碰撞率跨数量级变化明显，用对数轴更容易比较不同规模的收敛速度。
...
def main():
    ...
    # 输入路径下的每个子目录被视作一个规模组；当前脚本里 legend 仍保留源码给出的单一标签约定。
    ...
    plt.savefig(
        os.path.join(os.getcwd(), '../final_plots/quads_train_scaling.pdf'),
        format='pdf',
        bbox_inches='tight',
        pad_inches=0.01,
    )

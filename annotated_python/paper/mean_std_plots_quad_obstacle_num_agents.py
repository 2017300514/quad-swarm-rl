# 中文注释副本；原始文件：paper/mean_std_plots_quad_obstacle_num_agents.py
# 该脚本把障碍环境里不同 agent 数量的训练结果聚合成论文对比图。
# 上游输入是 `8_2`、`16_2`、`32_2` 等目录下的 TensorBoard 日志；下游输出是 `final_plots/scale_num_agents.pdf`，
# 用来比较不同 swarm 规模下的成功率、碰撞率以及两类到目标距离指标。
...
def extract(experiments):
    ...
    # 与其它论文作图脚本一致，先保留原始不规则 step，再统一重采样到固定网格。
    x_per_key = [
...
def aggregate(path, subpath, experiments, ax, legend_name, group_id):
    # 每个规模组分别缓存，便于反复调试 EMA/平滑参数时复用中间结果。
...
def smooth(y, radius, mode='two_sided', valid_only=False):
    # EMA 之后再做一次极短窗口平滑，用来抑制 TensorBoard 标量抖动。
    '''
...
def one_sided_ema(xolds, yolds, low=None, high=None, n=512, decay_steps=1., low_counts_threshold=1e-8):
    # 这个 EMA 把不规则日志重采样到等距网格，并保持“只看过去”的因果平滑。
    '''
...
def symmetric_ema(xolds, yolds, low=None, high=None, n=512, decay_steps=1., low_counts_threshold=1e-8):
    # 名称沿用源码，但当前实现依然只返回 one-sided EMA 结果。
    """
...
def plot(index, interpolated_key, ax, set_xlabel, legend_name, group_id, set_y_size):
    ...
    # 每条 run 先被重采样到统一 200 点网格，再做短窗口平滑，便于跨规模比较总体趋势。
    ...
    # 前两项是比例指标，固定到 [0, 1] 范围后更容易直接比较不同规模的训练效果。
...
def main():
    ...
    # 这里显式固定 8 / 16 / 32 agent 三个输入目录，说明脚本服务的是论文里的固定对比组。
    ...
    # 整张图共享一个 legend，标签本身就是不同的 agent 数量。

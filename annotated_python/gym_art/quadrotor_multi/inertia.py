...
# `rotate_I` 封装了当前模块中的一段独立流程，阅读时应重点关注它消费哪些状态、又把结果交给谁继续使用。
def rotate_I(I, R):
    # 下面开始文件或代码块自带的文档字符串；如果源码作者已经解释设计意图，应优先结合它理解上下文。
    """
...
# `translate_I` 封装了当前模块中的一段独立流程，阅读时应重点关注它消费哪些状态、又把结果交给谁继续使用。
...
# `deg2rad` 封装了当前模块中的一段独立流程，阅读时应重点关注它消费哪些状态、又把结果交给谁继续使用。
def deg2rad(deg):
    # 这里把当前阶段整理好的结果交还给上层调用者；真正要理解的是返回值之后会进入哪条训练或仿真链路。
    return deg / 180. * np.pi

# `SphereLink` 是当前文件暴露的核心类型，它负责维护与该模块职责直接相关的长期状态。
class SphereLink():
    # 下面开始文件或代码块自带的文档字符串；如果源码作者已经解释设计意图，应优先结合它理解上下文。
    """
    Box object
    ...
    @property
    # `I_com` 封装了当前模块中的一段独立流程，阅读时应重点关注它消费哪些状态、又把结果交给谁继续使用。
    def I_com(self):
        r = self.r
        # 这里把当前阶段整理好的结果交还给上层调用者；真正要理解的是返回值之后会进入哪条训练或仿真链路。
        return np.array([
    ...
    # `compute_m` 封装了当前模块中的一段独立流程，阅读时应重点关注它消费哪些状态、又把结果交给谁继续使用。
    def compute_m(self, density):
        # 这里把当前阶段整理好的结果交还给上层调用者；真正要理解的是返回值之后会进入哪条训练或仿真链路。
        return density * 4./3. * np.pi * self.r ** 3


# `BoxLink` 是当前文件暴露的核心类型，它负责维护与该模块职责直接相关的长期状态。
class BoxLink():
    # 下面开始文件或代码块自带的文档字符串；如果源码作者已经解释设计意图，应优先结合它理解上下文。
    """
    Box object
...
# `RodLink` 是当前文件暴露的核心类型，它负责维护与该模块职责直接相关的长期状态。
class RodLink():
    # 下面开始文件或代码块自带的文档字符串；如果源码作者已经解释设计意图，应优先结合它理解上下文。
    """
...
# `CylinderLink` 是当前文件暴露的核心类型，它负责维护与该模块职责直接相关的长期状态。
class CylinderLink():
    # 下面开始文件或代码块自带的文档字符串；如果源码作者已经解释设计意图，应优先结合它理解上下文。
    """
    Vertical Cylinder
...
# `LinkPose` 是当前文件暴露的核心类型，它负责维护与该模块职责直接相关的长期状态。
...
# `QuadLink` 是当前文件暴露的核心类型，它负责维护与该模块职责直接相关的长期状态。
class QuadLink(object):
    ...
    # 这里通过装饰器把额外框架语义附着到下面的定义上，真正影响的是后续调用方式或注册行为。
    @property
    # `m` 封装了当前模块中的一段独立流程，阅读时应重点关注它消费哪些状态、又把结果交给谁继续使用。
    def m(self):
        # 这里把当前阶段整理好的结果交还给上层调用者；真正要理解的是返回值之后会进入哪条训练或仿真链路。
        return np.sum([link.m for link in self.links]) 

# `QuadLinkSimplified` 是当前文件暴露的核心类型，它负责维护与该模块职责直接相关的长期状态。
...
if __name__ == "__main__":
    ...
    # `report` 封装了当前模块中的一段独立流程，阅读时应重点关注它消费哪些状态、又把结果交给谁继续使用。
    def report(quad):

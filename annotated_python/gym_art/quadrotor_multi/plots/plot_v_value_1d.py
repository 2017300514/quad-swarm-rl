# 中文注释副本；原始文件：gym_art/quadrotor_multi/plots/plot_v_value_1d.py
# 说明：为避免修改源码，本文件仅作为阅读辅助材料。
# 这个脚本是最轻量的离线 V-value debugger。
# 它不直接连环境，而是假定你已经在策略前向里手工扫过某一个观测维度，
# 然后把采样出的 1D value 曲线粘回来，用 Plotly 快速看 critic 对该维度的单变量形状。
...
# `x` 是手工扫过的某一维观测坐标，`y` 是同一组坐标上 critic 输出的 state value。
...
# 图标题里直接标出最大 value 及其对应坐标，便于肉眼定位 critic 在这个一维切片上偏好的区域。
xmax = x[np.argmax(y)]
ymax = y.max()

...
# 这个文件负责 domain randomization 里的机体参数采样。
# 上游给它名义参数或采样范围；下游是 `QuadrotorDynamics` / `quad_models` 真正消费的几何、质量、阻尼和电机参数字典。


def clip_params_positive(params):
    # 递归把一个参数子树裁到非负，避免几何尺寸和质量这类天然应为正的量被采样成非法值。
...
def check_quad_param_limits(params, params_init=None):
    # 这里是所有随机化结果进入动力学前的最后一道物理可行性约束。
    # 目标不是追求真实制造公差，而是防止训练时抽到明显无意义、会让积分崩掉的参数。
    ...
    if params_init is not None:
        # 当 thrust-to-weight 变化时，顺手按比例修正桨叶半径，保持电机能力和几何尺寸大致一致。
...
def get_dyn_randomization_params(quad_params, noise_ratio=0., noise_ratio_params=None):
    ...
    """
    # 先生成一份与参数树同构的“扰动幅度树”，后面采样时按同一层级逐项读取。
    ...
    # 允许对某些字段单独覆盖默认扰动幅度，例如只放大电机参数而保持质量更稳定。
...
def perturb_dyn_parameters(params, noise_params, sampler="normal"):
    ...
    # 采样完之后统一过约束，避免单次采样把参数推到动力学无法接受的区域。
...
def resample_dyn_parameters(params, noise_params, sampler="uniform"):
    ...
    Returns:
        dict: modified parameters
    """
    # 这一支是“按绝对范围重新采样一台新机体”，而不是围绕旧参数做比例扰动。
...
def randomquad_parameters():
    ...
    """
    # 这条采样路径生成结构较完整的随机四旋翼：
    # 会同时给出密度、几何尺寸、阻尼、推进系统参数，适合做更激进的 domain randomization。
    ###################################################################
...
def sample_t2w_t2t(params, t2w_min, t2w_max, t2t_min=0.003, t2t_max=0.009):
    # 同时调推力裕度和偏航力矩系数，直接影响控制 authority。
    params["motor"]["thrust_to_weight"] = np.random.uniform(low=t2w_min, high=t2w_max)
...
def sample_simplified_random_dyn():
    ...
    Returns:
        dict: sampled quadrotor parameters
    """
    # 这是更轻量的一条采样路径，只保留动力学主导量：
    # 总质量、臂长、推重比、电机时常等，适合简化模型或快速扫参数。
    ###################################################################
...
class RelativeSampler(object):
    # 先固定一套“相对扰动强度”，之后每次 sample 都围绕传入的当前参数做扰动。
...
class AbsoluteSampler(object):
    # 直接按绝对上下界重采一台机体，适合从一个更宽的可部署分布里抽样。
...
class ConstValueSampler(object):
    # 把某些字段强制覆盖成常量，通常作为实验对照或消融配置的小工具。
